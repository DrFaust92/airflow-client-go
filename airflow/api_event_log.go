...
// One or more event names separated by commas. If set, only return event logs with events matching this pattern. *New in version 2.9.0* 
func (r ApiGetEventLogsRequest) IncludedEvents(includedEvents string) ApiGetEventLogsRequest {
	r.includedEvents = &includedEvents
	return r
}

// One or more event names separated by commas. If set, only return event logs with events that do not match this pattern. *New in version 2.9.0* 
...
func (a *EventLogAPIService) GetEventLogsExecute(r ApiGetEventLogsRequest) (*EventLogCollection, *http.Response, error) {
	...
	if r.includedEvents != nil {
		parameterAddToHeaderOrQuery(localVarQueryParams, "included_events", r.includedEvents, "form", "")
	}
	if r.excludedEvents != nil {
		parameterAddToHeaderOrQuery(localVarQueryParams, "excluded_events", r.excludedEvents, "form", "")
	}
	// to determine the Content-Type header
	localVarHTTPContentTypes := []string{}
